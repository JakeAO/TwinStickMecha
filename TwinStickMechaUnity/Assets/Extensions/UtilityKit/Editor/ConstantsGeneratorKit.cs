...
namespace Prime31Editor
{
    ...
    public class ConstantsGeneratorKit : MonoBehaviour
    {
        ...
        public static void rebuildConstantsClasses(bool buildResources = true, bool buildScenes = true, bool buildTagsAndLayers = true, bool buildSortingLayers = true)
        {
            ...
            if (buildScenes)
            {
                File.WriteAllText(folderPath + SCENES_FILE_NAME, getScenesClassContent(SCENES_FILE_NAME.Replace(".cs", string.Empty), editorBuildSettingsScenesToNameStrings(EditorBuildSettings.scenes)));
        ...
        private static KeyValuePair<string, int>[] editorBuildSettingsScenesToNameStrings(EditorBuildSettingsScene[] scenes)
        {
            var sceneNames = new KeyValuePair<string, int>[scenes.Length];
            for (var n = 0; n < sceneNames.Length; n++)
                sceneNames[n] = new KeyValuePair<string, int>(Path.GetFileNameWithoutExtension(scenes[n].path), n);
        ...
        private static string getScenesClassContent(string className, KeyValuePair<string, int>[] sceneList)
        {
            var output = "";
            output += "//This class is auto-generated do not modify\n";
            output += "namespace " + NAMESPACE + "\n";
            ...
            foreach (var kvp in sceneList)
                output += "\t\t" + buildConstVariable(kvp.Key) + "\n";
            output += "\n";
            foreach (var kvp in sceneList)
                output += "\t\t" + buildConstVariable(kvp.Key, "_IDX", kvp.Value) + "\n";
        ...
        private static string buildConstVariable(string varName, string suffix, int value)
        {
            return "public const int " + formatConstVariableName(varName) + suffix + " = " + value + ";";
        }

        private static string buildConstVariable(string varName, string suffix, uint value)
        {
            return "public const uint " + formatConstVariableName(varName) + suffix + " = " + value + ";";
        }

        private static string buildConstVariable(string varName, string suffix, float value)
        {
            return "public const float " + formatConstVariableName(varName) + suffix + " = " + value + ";";
        }
